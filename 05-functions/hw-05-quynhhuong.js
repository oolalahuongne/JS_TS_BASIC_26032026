...
function taoPayloadDangNhap(formInput, options = {}) {
    let {username, password, role, rememberMe, device} = formInput; //object destructuring, let thì mới sửa được
    ...
    const finalRole = cleanRole === "" ? defaultRole : cleanRole;
    if (!allowedRoles.includes(finalRole)) {
        errors.push("role is not allowed");
    }
    let cleanRememberMe;
    if (typeof rememberMe === "boolean"){
        cleanRememberMe = rememberMe;
    } else { 
        const value = rememberMe.toString().trim().toLowerCase();
        if (value === "yes" || value === "on"){
            cleanRememberMe = true;
        } else {
            cleanRememberMe = false;
        }
    }
...
// console.log("case1", taoPayloadDangNhap(loginTestData[0].formInput, loginOptions));
// console.log("case2", taoPayloadDangNhap(loginTestData[1].formInput, loginOptions));
// console.log("case3", taoPayloadDangNhap(loginTestData[2].formInput, loginOptions));
// console.log("case4", taoPayloadDangNhap(loginTestData[3].formInput, loginOptions));
// console.log("case5", taoPayloadDangNhap(loginTestData[4].formInput, loginOptions));
// console.log("case6", taoPayloadDangNhap(loginTestData[5].formInput, loginOptions));
// console.log("case7", taoPayloadDangNhap(loginTestData[6].formInput, loginOptions));
// console.log("case8", taoPayloadDangNhap(loginTestData[7].formInput, loginOptions));
...
function chuanHoaDanhSachTest(rawRows, config = {}) {
    ...
    for (let item of rawRows) {
        const [id, module, priority, tag, status] = item; //array destructuring
        ...
        //bắt đầu bằng "TC_"
        if (!testCase.id.startsWith("TC_")) {
            isValid = false
        } 

        //trùng ID
        if (testCase.id.startsWith("TC_")) {
            if (seenIds.includes(testCase.id)) {
                isValid = false;
                duplicateIds++;
            } else {
                seenIds.push(testCase.id);
            }
        } 
        ...
        //Check priority trong config
        if (testCase.priority < minPriority || testCase.priority > maxPriority) {
            isValid = false
        }

        //Check status
        if (testCase.status !== "active" && testCase.status !== "inactive") {
            isValid = false
        }
...
function taoCauHinhCuoi(defaultConfig, envConfig, overrideConfig) {
    const configs = {...defaultConfig, ...envConfig, ...overrideConfig, reporter: {...defaultConfig.reporter, ...envConfig.reporter, ...overrideConfig.reporter}};
    return configs
...
function kiemTraCauHinh(config) {
    ...
    const cleanBrowsers = browsers.map((browser) => {
        return browser.trim().toLowerCase();
    })
    const duplicatedBrowsers = cleanBrowsers.filter((browser, index) => {
        return cleanBrowsers.indexOf(browser) !== index;
    })
    const firstDuplicatedBrowser = duplicatedBrowsers.find((browser) => {
        return browser
    })
    ...
    if (retries < 0) {
        errors.push("No Retries");
    }
    if (cleanBrowsers.length === 0) {
        errors.push("Browser cannot be empty");
    }
    if (firstDuplicatedBrowser) {
        errors.push("Duplicated Browser");
    }
    if (env === "ci" && headed === true) {
        warnings.push("Wrong env");
    }
...
allCases.forEach((configCase, index) => {
    const finalConfig = taoCauHinhCuoi(
        configCase.defaultConfig,
        configCase.envConfig,
        configCase.overrideConfig
    );
