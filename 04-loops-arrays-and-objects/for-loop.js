...
for (let sanPhamHetHang of sanPhams) {
  if (sanPhamHetHang.conHang === false) {
...
for (sanPhamConHang of sanPhams) {
  if (!sanPhamConHang.conHang) {
    continue
  }
  console.log(`${sanPhamConHang.ten}`);
}
...
for (let keys in userInfo) {
  if (fieldAnDi.includes(keys)) {
    continue
  }
  console.log(`${keys}:${userInfo[keys]}`);
}
...
const danhSachVip = [];
for (let price of danhSachGia) {
  if (price > 100000) {
    danhSachVip.push(price)
  } 
} 
